...
show_alloc(void)
{
    ...
    fprintf(stderr, "List allocations: %" PY_FORMAT_SIZE_T "d\n",
            count_alloc);
    fprintf(stderr, "List reuse through freelist: %" PY_FORMAT_SIZE_T "d\n", count_reuse);
...
#endif
/**
 * PyListObject 的内存缓冲池中最多有 80 个。
*/
static PyListObject *free_list[PyList_MAXFREELIST];
/**
 * free_list 中已经缓存的 PyListObject 对象的书目。
*/
...
PyObject *PyList_New(Py_ssize_t size)
{
    ...
    */
    ...
    /**
     * （2）创建连续的 size 个 PyObject 指针大小的内存。
    */
    if (size <= 0) // 空 list 。
        op->ob_item = NULL;
    else
...
/**
 * @function    申请一个包含对象指针数量为 size 的新的 list 。
 * @paras   size    对象指针的数量。
 * @ret 返回 list 对象指针。
*/
static PyObject *
...
list_dealloc(PyListObject *op)
{
    ...
    /**
     * （1）释放 list 的内存槽，降低各个对象指针指向的对象的引用计数。
    */
 ...
 * @paras   ilow    切片的起始位置。
 *          ihigh   切片的结束位置。
 * @ret 返回新建立的专门用于保存切片数据的 list 的首地址。
 * @notice  因为仅仅是拷贝了对象指针，所以本函数属于浅拷贝。
*/
static PyObject *
list_slice(PyListObject *a, Py_ssize_t ilow, Py_ssize_t ihigh)
{
    ...
    /**
     * src 指向源切片的起始位置，
     * dest 指向专门用于保存切片数据的 list 的二维数组的首地址
    */
    PyObject **src, **dest;
    Py_ssize_t i, len;
    /**
...
/**
 * @function    连接两个 list 。
 * @paras   a、b，待连接的两个 list 。
 * @ret 连接了两个 list 的 list 对象的首地址。
 * @notice  浅拷贝。
*/
static PyObject *
list_concat(PyListObject *a, PyObject *bb)
{
    ...
    Py_ssize_t i;
    /**
     * src  先后指向 a 的 ob_item 和 b 的 ob_item 。
     * dest 指向新的 list 的 ob_item 的不同位置。
    */
    PyObject **src, **dest;
    PyListObject *np;
    if (!PyList_Check(bb))
...
/*[clinic end generated code: output=57b9f9c5e23fbe42 input=cb56cd179a713060]*/
{
    ...
    if (saved_ob_size > 1)
    {
        ...
        PyTypeObject *key_type = (keys_are_in_tuples ? PyTuple_GET_ITEM(lo.keys[0], 0)->ob_type : lo.keys[0]->ob_type);
                                        ...
                                        LIST_INDEX_METHODDEF
                                            LIST_COUNT_METHODDEF
                                                LIST_REVERSE_METHODDEF
                                                    LIST_SORT_METHODDEF{NULL, NULL} /* sentinel */
};
