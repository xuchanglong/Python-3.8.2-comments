        ...
        GC_COLLECT_METHODDEF
            GC_GET_OBJECTS_METHODDEF
                GC_GET_STATS_METHODDEF
                    GC_IS_TRACKED_METHODDEF{"get_referrers", gc_get_referrers, METH_VARARGS,
...
void _PyGC_DumpShutdownStats(_PyRuntimeState *runtime)
{
    struct _gc_runtime_state *state = &runtime->gc;
    if (!(state->debug & DEBUG_SAVEALL) && state->garbage != NULL && PyList_GET_SIZE(state->garbage) > 0)
    {
        const char *message;
        if (state->debug & DEBUG_UNCOLLECTABLE)
...
void _PyGC_Fini(_PyRuntimeState *runtime)
{
    struct _gc_runtime_state *state = &runtime->gc;
